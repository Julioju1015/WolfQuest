...
module.exports = {
    name: 'new',
    description: 'Lancer une nouvelle partie de Cristal Lake',
        ...
        if (!LG.running) {
            ...
            launchNewGame(LGBot, message, LG).catch(err => {
                if (err.name === "DiscordAPIError") {
                    ...
                    LGBot.users.cache.find((user) => user.id === '723534138228867082').send(errMsg).catch(console.error)
                } else {
            ...
            message.channel.send("Partie de LG déjà en cours, pour stopper la partie de force, tapez /stop").catch(console.error);
        }
    },
};
